...
// Reversed jet colormap: red → yellow → green → cyan → blue
// Used for altitude limits where higher values (blue) are better
export function jetColormapReversed(value, min, max) {
  // Handle NaN and invalid values
  if (isNaN(value) || value === null) {
    return 'rgba(128, 128, 128, 0.3)'; // Gray for NaN
  }
  ...
  const t = 1 - Math.max(0, Math.min(1, (value - min) / (max - min)));
